...
1. One topic starts by loading persistent local context for the intended handle. Read topic-relevant operator revision records and treat prior imagery as a negative selection list before research, drafting, or generation.
...
- Preserve a specific `visualConcept` and the complete actual `generationPrompt` for every generated editorial image. Across different handles, exact/near-duplicate generated imagery and exact prompt reuse are hard blockers. Same-handle revisions may retain their images. Reusing Origin-owned or user-provided media across handles requires explicit manifest approval and a specific reason.
...
- `scripts/local_memory.py` locks persistent local operator-record discovery and cross-article image-use history.
...
- `scripts/prepare_bundle.py` locks tier length, experience ratio/evidence, structure, HTML safety, links, image manifest, persistent image-history checks, metadata, update target, hashes, and action-specific approval phrases.
