...
- Persistent local memory is missing, corrupt, unwritable, or was not loaded before image planning. Do not change the memory directory or erase history to bypass this blocker.
- A generated image lacks its specific `visualConcept` or complete actual `generationPrompt`, repeats a prompt used by another handle, or exactly/perceptually matches a generated image from another handle. Same-handle revisions are exempt from the cross-article reuse rule.
- An Origin-owned or user-provided image matches media used by another handle without `reuseApproved: true` and a specific `reuseReason`.
