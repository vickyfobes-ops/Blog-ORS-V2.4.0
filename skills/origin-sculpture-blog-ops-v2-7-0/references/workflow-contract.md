...
Before drafting or choosing images, run the persistent local context step in `local-learning-memory.md`. The local memory remains outside the installed Skill and survives version upgrades.
...
```json
{
  ...
  "blogHandle": "news",
  "title": "Public article title intended as the theme-rendered H1",
...
[
  {
    ...
    "visualConcept": "quiet architectural courtyard with one abstract focal sculpture",
    "generationPrompt": "Complete prompt actually used to generate this topic-specific editorial image...",
    "visualQa": {
      "inspected": true,
...
Every generated editorial entry also requires a specific `visualConcept` and the complete `generationPrompt` actually used. `prepare_bundle.py` stores both with byte and visual fingerprints in persistent local memory. Across different article handles, exact/near-duplicate generated imagery and exact prompt reuse are hard blockers. A revision with the same handle may keep its images. Origin-owned or user-provided media reused across handles requires `reuseApproved: true` plus a specific `reuseReason`; this exception never applies to generated editorial images. See `local-learning-memory.md`.
...
- Start with a readable `<p>` that answers the public title's main question or decision; aim for a self-contained 40–70 English words, and always give the answer in the first 80 words. Follow with H2s for distinct reader questions/decisions and H3s beneath their parent H2; do not start with H3.
- The article title appears in review `article.md`/Word and in `meta.json.title`, not as a second H1 in publishable HTML. This separation alone does not prove the storefront template actually renders the title as H1.
- Do not paste another site's wrapper/class (for example a WordPress `senyang-article` div) or page-level font/line-height styling into Shopify HTML. Use the selected theme's CSS for consistent presentation; treat any article-specific callout as a reviewed exception.
...
Before live publication, inspect a current public article known to use the same selected article template, and verify the rendered HTML has one H1 containing that article's public title. Record the URL, template identity, date, and H1 in review notes. Recheck after a theme/template change. If there is no matching page or the result is zero/multiple H1s, do not publish live or insert an H1 in the body; ask for a theme correction. Saving an explicitly authorized unpublished draft is still allowed if the reviewer is told it must not be manually published until this is resolved.

## Approval binding

`prepare_bundle.py` verifies that the public article text in `article.md` matches `article.html`, checks persistent local image history, adds the approved/inferred Blog Filter to the publication payload, then hashes `meta.json`, `article.md`, `article.html`, `summary.html`, `sources.md`, and `link-plan.json`. When present, it also hashes `image-assets.json`, every referenced WebP, and `update-target.json`. A passing bundle is recorded in the local image ledger before an approval phrase is issued. `shopify_publish.py` rechecks the bundle hashes. Editing any source file, approved image, or captured update target invalidates the confirmation phrase.
