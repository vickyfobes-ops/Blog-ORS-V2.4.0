...
from local_memory import (
    audit_bundle as audit_image_memory,
    bootstrap_memory,
    default_memory_dir,
    record_prepared_entries,
    roots_for_bundle,
)
...
USER_AGENT = "OriginSculptureBlogSkill/2.7.0 (+https://originsculpture.com)"
...
def main() -> int:
    ...
    image_memory_entries: list[dict] = []
    image_memory_report: dict[str, object] = {
        "status": "NOT_RUN",
        "memoryDir": str(default_memory_dir()),
    }
    if image_manifest_path.is_file():
        try:
            ...
            for index, item in enumerate(image_assets):
                ...
                visual_concept = str(item.get("visualConcept", "")).strip()
                generation_prompt = str(item.get("generationPrompt", "")).strip()
                ...
                if source_type == "generated-editorial":
                    if len(visual_concept) < 8:
                        blockers.append(
                            f"generated image asset '{slot}' needs a specific visualConcept for local reuse checks"
                        )
                    if len(generation_prompt) < 40:
                        blockers.append(
                            f"generated image asset '{slot}' needs the complete generationPrompt used to create it"
                        )
                ...
                if visual_concept:
                    item["visualConcept"] = visual_concept
                if generation_prompt:
                    item["generationPrompt"] = generation_prompt
                if source_page:
    ...
    if asset_by_slot and handle:
        try:
            memory_dir = default_memory_dir()
            bootstrap_memory(
                roots_for_bundle(bundle),
                memory_dir=memory_dir,
                exclude_bundle=bundle,
            )
            image_memory_report, image_memory_entries = audit_image_memory(
                bundle,
                handle=handle,
                assets=list(asset_by_slot.values()),
                memory_dir=memory_dir,
            )
            blockers.extend(str(value) for value in image_memory_report.get("blockers", []))
            warnings.extend(str(value) for value in image_memory_report.get("warnings", []))
        except Exception as exc:
            image_memory_report = {
                "status": "BLOCKED",
                "memoryDir": str(default_memory_dir()),
                "blockers": [f"local image memory unavailable: {exc}"],
                "warnings": [],
            }
            blockers.extend(image_memory_report["blockers"])
    ...
    if not blockers and image_memory_entries:
        try:
            image_memory_report["record"] = record_prepared_entries(
                image_memory_entries,
                memory_dir=default_memory_dir(),
            )
        except Exception as exc:
            message = f"could not update the persistent local image ledger: {exc}"
            blockers.append(message)
            image_memory_report["status"] = "BLOCKED"
            image_memory_report.setdefault("blockers", []).append(message)
    ...
    qa = {
        ...
        "localMemory": image_memory_report,
