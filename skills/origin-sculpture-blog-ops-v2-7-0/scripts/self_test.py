    ...
    assets = [
        {
            ...
            "visualConcept": "neutral architectural cover scene with one abstract sculpture",
            "generationPrompt": "Create a wide photorealistic editorial scene of one abstract sculpture in a neutral architectural setting, with no text, logos, screens, or project attribution.",
            "visualQa": qa_true,
        },
        {
            "slot": "site-scene",
            ...
            "sourceType": "generated-editorial",
            "visualRole": "environment-scene",
            "visualConcept": "contemporary courtyard site-planning scene",
            "generationPrompt": "Create a wide photorealistic editorial view of an abstract metal sculpture in a contemporary courtyard for site-planning discussion, with realistic scale and no text or logos.",
            "visualQa": qa_true,
        },
        {
            "slot": "approval-scene",
            ...
            "visualConcept": "finish-sample approval scene beside a maquette",
            "generationPrompt": "Create a wide photorealistic studio process scene showing finish samples reviewed beside a sculpture maquette, with realistic materials and no labels, text, screens, or logos.",
            "visualQa": qa_true,
        },
        {
        ...
        },
        {
            ...
            "sourceType": "generated-editorial",
            "visualRole": "process-scene",
            "visualConcept": "close surface inspection in a maintenance setting",
            "generationPrompt": "Create a wide photorealistic editorial process scene of a sculpture surface being inspected in a clear maintenance setting, with realistic tools, no text, no screen, and no logo.",
            "visualQa": qa_true,
        },
        {
            "slot": "project-review",
...
def run_self_test(work_dir: Path, renderer: Path) -> dict[str, object]:
    ...
    same_handle_result = run([sys.executable, str(prepare), str(same_handle_bundle)])
    if json.loads(same_handle_result.stdout).get("status") != "PASS":
        raise SelfTestError("local image ledger rejected an allowed same-handle revision")

    duplicate_image_bundle = work_dir / "cross-handle-duplicate-images"
    shutil.copytree(bundle, duplicate_image_bundle)
    duplicate_meta_path = duplicate_image_bundle / "meta.json"
    duplicate_meta = json.loads(duplicate_meta_path.read_text(encoding="utf-8"))
    duplicate_meta["handle"] = "origin-skill-cross-handle-duplicate"
    duplicate_meta_path.write_text(json.dumps(duplicate_meta, indent=2) + "\n", encoding="utf-8")
    duplicate_review_path = duplicate_image_bundle / "article.md"
    duplicate_review_path.write_text(
        duplicate_review_path.read_text(encoding="utf-8").replace(
            "origin-skill-self-test", "origin-skill-cross-handle-duplicate"
        ),
        encoding="utf-8",
    )
    duplicate_result = subprocess.run(
        [sys.executable, str(prepare), str(duplicate_image_bundle)],
        text=True,
        capture_output=True,
        check=False,
    )
    duplicate_output = duplicate_result.stdout + duplicate_result.stderr
    if duplicate_result.returncode == 0 or "generated image asset" not in duplicate_output:
        raise SelfTestError("local image ledger failed to reject exact generated-image reuse across handles")

    near_duplicate_bundle = work_dir / "cross-handle-near-duplicate-images"
    shutil.copytree(bundle, near_duplicate_bundle)
    near_meta_path = near_duplicate_bundle / "meta.json"
    near_meta = json.loads(near_meta_path.read_text(encoding="utf-8"))
    near_meta["handle"] = "origin-skill-cross-handle-near-duplicate"
    near_meta_path.write_text(json.dumps(near_meta, indent=2) + "\n", encoding="utf-8")
    near_review_path = near_duplicate_bundle / "article.md"
    near_review_path.write_text(
        near_review_path.read_text(encoding="utf-8").replace(
            "origin-skill-self-test", "origin-skill-cross-handle-near-duplicate"
        ),
        encoding="utf-8",
    )
    near_assets_path = near_duplicate_bundle / "image-assets.json"
    near_assets = json.loads(near_assets_path.read_text(encoding="utf-8"))
    from PIL import Image, ImageDraw

    for index, asset in enumerate(near_assets):
        if asset.get("sourceType") == "generated-editorial":
            asset["generationPrompt"] = f"A deliberately distinct self-test prompt number {index} that still verifies perceptual duplicate rejection after a tiny pixel-level image change."
            for key, image_format in (("png", "PNG"), ("webp", "WEBP")):
                image_path = near_duplicate_bundle / asset[key]
                with Image.open(image_path) as image:
                    changed = image.convert("RGB")
                ImageDraw.Draw(changed).rectangle((1597, 897, 1599, 899), fill=(1 + index, 2, 3))
                if image_format == "WEBP":
                    changed.save(image_path, format=image_format, quality=92, method=6)
                else:
                    changed.save(image_path, format=image_format)
        else:
            asset["reuseApproved"] = True
            asset["reuseReason"] = "Self-test exception to isolate the generated-image near-duplicate gate."
    near_assets_path.write_text(json.dumps(near_assets, indent=2) + "\n", encoding="utf-8")
    near_result = subprocess.run(
        [sys.executable, str(prepare), str(near_duplicate_bundle)],
        text=True,
        capture_output=True,
        check=False,
    )
    near_output = near_result.stdout + near_result.stderr
    if near_result.returncode == 0 or "visually matches a prior image" not in near_output:
        raise SelfTestError("local image ledger failed to reject perceptually similar generated images")
    ...
    invalid_structure_bundle = work_dir / "invalid-structure-bundle"
    shutil.copytree(bundle, invalid_structure_bundle)
    article_path = invalid_structure_bundle / "article.html"
    valid_body = article_path.read_text(encoding="utf-8")
    for invalid_body, expected in (
        ("<h1>Duplicate title</h1>\n" + valid_body, "article.html must not contain H1"),
        ('<div class="senyang-article">\n' + valid_body + "</div>\n", "must begin with a readable answer paragraph"),
        ("<p>Short opening.</p>\n<h3>Premature detail</h3>\n" + valid_body, "must not use H3 before its parent H2"),
    ):
        article_path.write_text(invalid_body, encoding="utf-8")
        structural_result = subprocess.run(
            [sys.executable, str(prepare), str(invalid_structure_bundle)],
            text=True,
            capture_output=True,
            check=False,
        )
        if structural_result.returncode == 0 or expected not in (structural_result.stdout + structural_result.stderr):
            raise SelfTestError(f"publication gate failed to reject bad HTML structure: {expected}")
    ...
    return {
        ...
        "duplicateBodyH1Rejected": True,
        "foreignOuterWrapperRejected": True,
        "prematureH3Rejected": True,
